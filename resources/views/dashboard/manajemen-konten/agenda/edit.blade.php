                ...
                <div class="card card-flush h-md-100">
                  <div class="card-body d-flex flex-column justify-content-between bgi-no-repeat bgi-size-cover bgi-position-x-center pb-0">
                    <div>
                      ...
                      @endforeach
                      @endif
                      <form class="form" action="/agenda/{{$agendas->id}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="d-flex flex-column mb-6 fv-row">
                          ...
                          <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" value="{{old('judul',$agendas->judul)}}" />
                        </div>
                        <div class="d-flex flex-column mb-6 fv-row">
                          <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                            <span>Slug <span class="text-primary">(Optional)</span></span>
                          </label>
                          <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" value="{{old('slug',$agendas->slug)}}" />
                        ...
                        <div class="d-flex flex-column mb-6 fv-row">
                          ...
                          <select class="form-select" data-control="select2" data-hide-search="true" data-placeholder="Pilih Unit Kerja" name="unitKerja" id="unitKerja">
                            ...
                            @if (old('unitKerja_id',$agendas->unitKerja_id)==$unitKerja->id)
                        ...
                        <div class="row">
                          ...
                          <div class="col-5 ms-5">
                            ...
                            <div class="d-flex flex-column mb-6 fv-row">
                              <input type="datetime-local" class="form-control @error('tanggal_mulai') is-invalid @enderror" name="tanggal_mulai" value="{{old('tanggal_mulai',$agendas->tanggal_mulai)}}" />
                            </div>
                          </div>
                          <div class="col-5">
                            <label class="d-flex align-items-center fs-6 fw-semibold mb-2">Tanggal Selesai :</label>
                            <div class="d-flex flex-column mb-6 fv-row">
                              <input type="datetime-local" class="form-control @error('tanggal_selesai') is-invalid @enderror" name="tanggal_selesai" value="{{old('tanggal_selesai',$agendas->tanggal_selesai)}}" />
                            </div>
                          </div>
                        </div>
                        <div class="d-flex flex-column mb-6 fv-row">
                          ...
                          <input type="text" class="form-control @error('lokasi') is-invalid @enderror" name="lokasi" value="{{old('lokasi',$agendas->lokasi)}}" />
                        </div>
                        <div class="d-flex flex-column mb-6 fv-row">
                          <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                            <span>Deskripsi <span class="text-primary">(Optional)</span></span>
                          </label>
                          <textarea name="deskripsi" id="deskripsi" cols="50" rows="10" class="form-control @error('deskripsi') is-invalid @enderror">{{old('deskripsi',$agendas->deskripsi)}}</textarea>
                        </div>
                        <div class="d-flex flex-column mb-6 fv-row">
                          <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                            <span>Tautan <span class="text-primary">(Optional)</span></span>
                          </label>
                          <input type="text" class="form-control @error('tautan') is-invalid @enderror" name="tautan" value="{{old('tautan',$agendas->tautan)}}" />
                        </div>
                        <div class="d-flex flex-column mb-6 fv-row">
                          <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                            <span>Email <span class="text-primary">(Optional)</span></span>
                          </label>
                          <input type="text" class="form-control @error('surel') is-invalid @enderror" name="surel" value="{{old('surel',$agendas->surel)}}" />
                        </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      <div class="d-flex flex-column mb-6 fv-row">
                        ...
                        <select class="form-select" data-control="select2" data-hide-search="true" data-placeholder="Pilih Status" name="status" id="status">
                          <option value="published" {{ $agendas->status == 'published' ? 'selected' : '' }}>Published</option>
                          <option value="inactive" {{ $agendas->status == 'inactive' ? 'selected' : '' }}>Inactive</option>
                          <option value="draft" {{ $agendas->status == 'draft' ? 'selected' : '' }}>Draft</option>
                        </select>
                      </div>
                      <div class="d-flex flex-column mb-6 fv-row">
                        <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                        ...
                        <div class="col-lg-8">
                          <div class="image-input image-input-outline" data-kt-image-input="true" style="background-image: url('/media/images/blank.jpg')">
                            <div class="image-input-wrapper w-175px h-125px" style="background-image: url('/media/gambar/{{ $agendas->gambar }}')"></div>
                            <label class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow" data-kt-image-input-action="change" data-bs-toggle="tooltip" title="Upload gambar">
                              ...
                              <input type="file" name="gambar" accept=".png, .jpg, .jpeg" value="{{old('gambar')}}" />
                            </label>
