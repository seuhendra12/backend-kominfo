					...
					<div class="col-xxl-6">
						<div class="card card-flush h-md-100">
							<div class="card-body d-flex flex-column justify-content-between bgi-no-repeat bgi-size-cover bgi-position-x-center pb-0">
								<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
									<h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Ubah Data Role</h1>
								</div>
								<hr>
								<div>
									@if($errors->any())
									...
									@endforeach
									@endif
									<form class="form" action="/data-role/{{$roles->id}}" method="POST">
										@csrf
										@method('PUT')
										<div class="d-flex flex-column mb-8 fv-row">
											...
											<input type="text" class="form-control" placeholder="Masukkan Nama Role" name="name" value="{{old('name',$roles->name)}}" />
										</div>
										...
										<div class="border mb-8 vh-50">
											<div class="row p-3">
												...
												<div class="col-3 mb-2">
													<label class="form-check form-check-custom form-check-solid">
														<input class="form-check-input h-20px w-20px me-2 mb-2" type="checkbox" name="permissions[]" value="{{$permission->id}}" {{ in_array($permission->id, $selected_permissions) ? 'checked' : '' }} /> {{$permission->name}}
													</label>
												</div>
												@endforeach
											</div>
